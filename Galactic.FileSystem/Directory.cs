...
namespace Galactic.FileSystem
{
    ...
    public static class Directory
    {
        ...
        static public bool GiveAccess(DirectoryEntry entry, string path, ref DirectorySecurity security, FileSystemRights rights,
            bool applyToSubfolders, bool applyToFiles, bool commitChanges)
        {
            // Check whether the entry, path, and security object are supplied.
            if (entry != null && !string.IsNullOrEmpty(path) && security != null)
            {
                // An entry, path, and security object are supplied.
        ...
        public static bool GiveAccess(AccessRights rights, DirectoryEntry entry, string path, ref DirectorySecurity security, bool applyToSubfolders, bool applyToFiles, bool commitChanges)
        {
            ...
            {
                if (!GiveAccess(entry, path, ref security, right, applyToSubfolders, applyToFiles, commitChanges))
                {
